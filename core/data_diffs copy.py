...
class DataDiff:
    ...
    def analyze_and_categorize_mismatches(self, mismatch_df, key_columns, output_file=None, 
        ...
        try:
            ...
            if not mismatch_df.empty:
                ...
                #self.log.info(f"Sample of mismatch DF:\n{mismatch_df.head(2)}")
            ...
            # Check if result_df is empty but we expect mismatches
            if result_df.empty and (len(source_rows) > 0 or len(target_rows) > 0):
            ...
            if not result_df.empty and '__key' in result_df.columns:
                result_df = result_df.drop(columns=['__key'])
    ...
    def _process_mismatches(self, source_rows, target_rows, key_columns, source_col,
        ...
        try:
            # Get unique keys 
            source_keys = set(source_rows['__key'])
            target_keys = set(target_rows['__key'])
            ...
            # One row per key for source-only, up to max_source_only
            source_only_count = 0
            ...
            # One row per key for target-only, up to max_target_only
            target_only_count = 0
            ...
            #self.log.info(f"Captured {len(mismatch_details)} unique key combinations with mismatches")
    ...
    def _export_to_excel(self, result_df, mismatch_info, output_file):
       ...
       try:
            ...
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                ...
                if '__key' in output_df.columns:
                    output_df = output_df.drop(columns=['__key'])
        ...
        try:
            ...
            if cmp_summary_df is not None:
                try:
                    ...
                    # Recalculate total differences based on our potentially updated value mismatch count
                    provided_total_diff = summary_dict.get('Total Differences', 0)
                    source_only = summary_dict.get('Rows Only in Source', source_only_count)
                    target_only = summary_dict.get('Rows Only in Target', target_only_count)
                    final_total_diff = final_value_mismatches + source_only + target_only
                    ...
                    self.comparison_summary = {
                        ...
                        'Rows Only in Source': source_only,
                        'Rows Only in Target': target_only,
    ...
    # Define get_column_letter if it's not available
    # This would normally be imported from openpyxl.utils
